...
struct iqte{ /* interal q tumbler element */
            struct queue q;
            tumbler t;
            long start;
};
...
#ifndef NDEBUG
#define _assert(ex) {if (!(ex)){fprintf(stderr,"Assertion ex failed: file %s, l\ine %d\n", __FILE__, __LINE__);abort();}}
#define assert(ex) {if (!(ex)){fprintf(stderr,"Assertion ex failed: file %s, li\ne %d\n", __FILE__, __LINE__);abort();}}
#else
#define _assert(ex) ;
#define assert(ex) ;
#endif
...
#ifndef NDEBUG
#define SMARTALLOC   1             /* Orphaned buffer detection */
#endif

/*  On virtual memory systems, if SMARTALLOC is set we pass an explicit
        string with the file and line number on every call to ALLOC rather
	    than use the hard code return address.  */

#ifdef SMARTALLOC
#ifdef CC
extern char *palloc(char * fname, int lineno, unsigned nbytes);
#else
extern char *palloc();
#endif
#define alloc(x) palloc(__FILE__, __LINE__, (x))

/*  FREE is redefined to call MEMFREE, our internal free routine which
        monitors available storage.  */

#ifdef CC
extern void memfree(char *);
#else
extern void memfree();
#endif
#define free(x)  memfree((x))

/*  MALLOC is defined to cause compile-time
        catastrophe.  You should use ALLOC instead.  */
...
/*  Protocol syntax defininitions  */

#define XDELIM      '~'            /* Xanadu token delimiter */
#define XNEWLINE    '\n'           /* Xanadu new line delimiter */
#define XBMAX       1024           /* Link communications buffer */
#define XTDELIM     '.'            /* Tumbler digit delimiter */
#define XERROR      '?'            /* Backend error signal */
