...
- xfollowlink:sender
{
 id newTextView;
 float h, w;
    id curTextView = [[NXApp mainWindow] contentView];
 ...
 id dv ;
  NXRect cur;
    ...
    if (wRect.origin.y < 0) {wRect.origin.y = ORIGY; wRect.origin.x = ORIGX;}
    newTextView = [TextView newFrame:&wRect];
//    [newTextView setDocument:&variabledocvposition];

    [[newTextView window] setDelegate:self];
	dv = [newTextView docView];
	 [dv getFrame:&cur];
    if(![newTextView followlink:curTextView]){
	// make frame go away
	}

    [dv getMinWidth:&w minHeight:&h maxWidth:cur.size.width maxHeight:1.0e38];
    [dv sizeTo:w :h];
...
{    id newTextView;
 ...
 id dv ;
  NXRect cur;
    ...
    if (wRect.origin.y < 0) {wRect.origin.y = ORIGY; wRect.origin.x = ORIGX;}
    newTextView = [TextView newFrame:&wRect];		
    [newTextView setDocument:variabledocvpositionp];
    [[newTextView window] setDelegate:self];     
	dv = [newTextView docView];
	 [dv getFrame:&cur];

    [newTextView readXanadu];
    [dv getMinWidth:&w minHeight:&h maxWidth:cur.size.width maxHeight:1.0e38];
    [dv sizeTo:w :h];
    return self;  
}
...
-terminate:sender
{
  ...
  */
 [NXApp getScreenList:&screenList count:&numWindows];	
    if (offset )
	for (offset--; offset >= 0; offset--){
//	    [[windowList at:offset] close];
//	theWindow = [windowList at:offset];
	theWindow = (id) screenList[offset];
		if(theWindow){
			viewtemp = [theWindow contentView];
			  if([viewtemp isKindOf:TextView]){
				addtostartqueue([viewtemp getDocument]);
	...
	free(screenList);
	//save queue to rc file
	savehomestorcfile();
	savepositionstorcfile();
	[NXApp terminate:self]; // go away
